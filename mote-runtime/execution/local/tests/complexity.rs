...
//!
//! This intentionally uses a small dependency-free scanner while the package
//! has no runtime dependencies. The baseline is exact: both growth and an
//! unrecorded reduction fail, so the metric cannot drift silently.
