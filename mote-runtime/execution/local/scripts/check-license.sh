...
grep -q 'Apache License' "$ROOT/LICENSE" || { printf 'license: LICENSE is not Apache-2.0 text\n' >&2; exit 1; }
grep -q 'Version 2.0' "$ROOT/LICENSE" || { printf 'license: Apache version marker is missing\n' >&2; exit 1; }
grep -Eq '^license = "Apache-2.0"$' "$ROOT/Cargo.toml" || {
    printf 'license: Cargo.toml license metadata must be Apache-2.0\n' >&2
    exit 1
}
grep -q 'Mote Local Execution' "$ROOT/NOTICE" || {
