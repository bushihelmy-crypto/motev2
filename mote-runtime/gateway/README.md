...
│   │   ├── protocol/            # adapter contract, facts, concrete adapters
│   │   ├── service/             # connector contract, facts, concrete connectors
...
│   │   └── upstream/            # network contracts and concrete transports
...
Inbound invocation infrastructure validates that contract and hands Gateway a
canonical `LLMRequestFrame` or `MediaRequestFrame`. Gateway execution APIs do
not accept raw request DTOs or infer optional-field presence.
...
The runtime has no upstream SDK dependency. The integration conformance runner
uses the pinned `jsonschema/v6` library so every vector is checked against the
authoritative repository schema; it is not part of the outbound adapter
surface. Add further dependencies only when a concrete owner and consumer need
them, then commit the resulting module metadata and update the relevant quality
gate.
...
- Kernel Think calls the LLM delivery entries; Execution calls the media
  delivery entries. Both reuse one configured `Invocation`, receive only their
  corresponding validated frame, and cannot interchange the two DTO profiles.
...
- `internal/upstream/` owns outbound network mechanics and connection reuse.
