...
composition root 在装配时直接把一个不可变 `HookPlan` 传给 HookNode，P1、P2
使用其中各自的 priority plan。Kernel 固定 P1 → P2 的节点顺序；
...
- HookPlan：composition root 直接传入的装配期执行计划；
...
Plan 本身已经完整表达要交给 Port 的配置，不需要 config source、loader 或 marker。
Graph 的执行坐标和恢复信息也不需要复制进 Hook 契约。

## 2. 装配期 Plan

HookNode 不读取配置、不订阅更新，也不维护 config source、plan loader 或热加载注册表。
composition root 负责构造完整 `HookPlan` 并直接传入 HookNode；同一 HookNode 的每次激活
都使用这一个不可变 Plan。需要更换 Plan 时，由 composition root 装配新的 HookNode。

### 2.1 HookPlan

HookPlan 是 HookNode 使用的不可变值，包含 P1、P2 对应的
HookPriorityPlan：
...
HookPlan 在 HookNode 装配前完成构造，不是运行期动态生成的 handler 清单。

### 2.2 HookPriorityPlan

HookPriorityPlan 只承载对应优先级的配置。内部 Port 将
...
- P1、P2 是两个确定的优先级节点；
- P1、P2 共享装配时直接传入的同一个 HookPlan，并严格按顺序推进；
- 上一优先级节点的最终类型化结果是下一优先级节点的输入；
- 任何 P1 command 或结果都不能让 P2 短路，P2 才能结束该子图；
- 两个节点都是普通图节点。

两个节点在构造期间完成组装；拓扑冻结沿用 Graph 首次成功编译后的既有边界。本轮不在
...
capability，然后构造同一个 Port 供 P1、P2 使用；缺少 invocation capability 时
立即装配失败。
...
`HookPayloadAdmission`，声明 `priority_config_type`、`value_type`、`state_type` 和
`command_type` 四个具体 nominal class。装配期 Plan、内部 Port 和 P2
最终结果都复用这份 contract：按 `type(payload) is declared_type` 检查 value、state、
priority config 及每个 command 元素，并要求 stage/final commands 为 exact
...
| HookPriority | 固定 P1/P2 节点身份，不描述节点内部调度 |
...
hooks/
  ...
  contract.py   # 请求、阶段/最终结果及 Plan admission
  plan.py       # HookPlan 和 HookPriorityPlan
  port.py       # HookPort：Hook-specific request 的 Invocation 适配（包级不导出）
  node.py       # HookNode 的 P1/P2 节点组合
~~~
...
阶段/最终结果以及 Plan 契约。`mote_kernel.hooks` 包级公共入口仍只有 HookNode。
...
Hooks 不拥有配置、路由、EventBus、统一状态 reducer、模型路由或 failover
实现。
