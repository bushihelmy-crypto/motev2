...
- `FrontierTransitionPlan` 是 runtime/recovery 共用的唯一 frontier/session/resource/routing/join/nested/planner-limit/completion lowering，并持有 entries、materialization、outcome/publication、graph completion 与 resource order；callable/nested classification 分别从 `CompiledGraph.nodes` nominal definitions 与 `nested_graphs` keys 派生，不重复存储；`RecoveryAvailabilityPlan` 只传播 canonical availability coordinates，不自建 edge map、resolver、scheduler、barrier 或 limits policy；recovery 使用 canonical completion、linear child proof 与 4096-state pre-mutation safety budget；
