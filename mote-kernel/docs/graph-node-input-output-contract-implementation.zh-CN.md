...
| recovery availability preflight | public family facade + shared `FrontierTransitionPlan`/`RecoveryAvailabilityPlan` | exact State settlements、effective limits、admitted resume/skip semantics、完整 frontier quiescence、parallel/resource/nested barrier、Result/limit boundary、scoped-run frame coordinates、full control/availability/child/action equality、canonical success-route requirements 与 failure/interrupt awaiting invariant | 预测 callable concrete output、shortened dedup identity、concrete-value comparison、descriptor/path-only runtime key、另建 routing/join/session/limits 解释器 |
...
7. 为每个 ordinary node 生成唯一 outcome-admission plan：conditional source 只接收 declared `SelectGraphRoute`，non-conditional source 只接收 `ContinueGraphRouting`，failure/interrupt 不伪造 route；
...
10. `RecoveryAvailabilityPlan` 只引用 shared transition/materialization/publication/outcome-admission plans 并附加 availability transfer：current State 中已有的 settlement 与已 admission 的 concrete skip 使用唯一 contribution；resume 后重新成为 Pending 的 conditional callable 与其他尚未执行的 conditional callable 一样，其 canonical success projection 包含全部 declared route branches；
...
OutcomeAdmissionPlan[GraphValueT]
PublicationPlan[GraphValueT]
FrontierTransitionPlan[GraphValueT]
RecoveryAvailabilityPlan[GraphValueT]
...
3. complete/recovered continuation variant、graph-input/publication frame、child snapshot pairing 和 `RecoveryAvailabilityPlan` 都是 execution 的 transient value/recovery proof，不是新的 durable control fact。
...
construct/validate one effective ExecutionLimits before compile/cache or mutation
    ...
    -> whole-invocation RecoveryAvailabilityPlan over shared FrontierTransitionPlan with the same limits when lineage is recovered
...
3. 实现 recovered-lineage whole-invocation `RecoveryAvailabilityPlan` evaluation：seed 先从 exact State/parent activation 导出 `ScopeRunCoordinate`，再从 admitted concrete frames 投影 coordinates；seen 以 full scoped-run control/availability/child/action equality 去重，sort-only traversal key 不合并 collision；current settlement/concrete skip 只沿唯一 route，failure/interrupt 消费完整 frontier；worklist 在首个 Result/limit 终止。
...
- `FrontierTransitionPlan` 是 runtime/recovery 共用的唯一 frontier/session/resource/routing/join/nested/planner-limit/completion lowering，并持有 entries、node classification、materialization、outcome/publication、graph completion 与 resource order；`RecoveryAvailabilityPlan` 只传播 canonical availability coordinates，不自建 edge map、resolver、scheduler、barrier 或 limits policy；recovery 使用 canonical completion、linear child proof 与 4096-state pre-mutation safety budget；
