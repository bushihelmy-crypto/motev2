...
- **复杂度门禁是高召回雷达**：热点命中只要求复审真实调用链，不直接判定代码有缺陷；指标没有设计裁决权，不能驱动假抽象、薄转发或状态机碎片化。
...
- 曾有 **4 项标为 `B`**（存在局部整理方向）；#27/#28/#32/#33 已完成并验收，本轮没有留下待实施的 B 项。
...
报告中的指标顺序固定为 `cc / cog / nest / params / nodes`。复杂度门禁有意追求高召回：一次命中表示“这里值得人工审查”，不表示“这里必须拆”。最终结论取决于 owner 是否唯一、状态与提交边界是否直接、规则是否重复以及完整调用链是否更易读。`pyproject.toml` 的 ratchet 当前几乎全部等于实际值；真正降低指标后必须同步下调上限，不能只让门禁继续接受旧数字。函数拆分也可能增加定义数，因此要看净变化，不把“多了几个 helper”当作简化。
...
| 6 | `execution/engine/recovery.py:729` `_nested_outcome_plans`（15/20/2/6/227） | 组合多个 nested child 的 completed/failed/aborted/waiting/limit 替代路径。 | **K**：组合爆炸是恢复证明本身，不是重复代码；不能以单一路径或隐式 fallback 取代。 |
...
| 11 | `execution/engine/resume_admission.py:76` `prepare_resume`（基线 21/40/4/2/471） | **基线历史职责**：曾同时构造 failed/interrupt resume command、resume input 和 skip substitution。 | **A（历史治理完成，2026-08-31；terminal FAILED 切片后再次收窄）**：先删除手工 frontier 模拟；随后 failed/skip/substitution 全部净删除。现行 `prepare_resume` 只处理 exact interrupt action、codec 和 input evidence。 |
| 12 | `execution/engine/resume_admission.py:190` `admit_resume_candidates`（基线 40/50/3/2/499 → 31/38/3/2/419） | **基线历史职责**：曾跨 scope 校验 failed/skip successor、substitution 和 publication collision。 | **A（历史治理完成，2026-08-31；terminal FAILED 切片后净删除）**：原有重复证明先被收敛；旧动作集合删除后该函数不再有独立职责，生产定义已删除，未保留 wrapper 或兼容入口。 |
...
| 23 | `execution/family_driver.py:558` `_GraphRun._drive_child`（12/12/2/2/139） | 读取 child handle 结果，检查 awaiting/completed/failed/aborted 类型对应关系并安装 boundary。 | **K**：这些分支是 terminal projection 不变量，不是可合并的重复路径。 |
...
| 44 | `state/graph_state/recovery_transitions.py:29` `resume_graph_nodes`（基线 22/34/5/2/258） | 只校验 exact interrupt resume action，并按 node identity 与 interrupt ID 更新 frontier。 | **K（terminal FAILED 切片后收窄）**：failed/skip action 与兼容分支已经删除；State 仍是 interrupt 恢复转换的唯一 owner，Execution 不能复制一份 reducer。 |
...
| 48 | `state/graph_state/validation.py:71` `validate_graph_frontier`（18/23/4/2/205） | 校验每个 frontier settlement、routing、failure、interrupt payload 和 codec 必要性。 | **K**：这是 durable frontier 的总不变量，分支对应 nominal settlement 类型。 |
| 49 | `state/graph_state/validation.py:125` `validate_graph_run_state`（33/43/2/1/357） | 校验 parent identity、join progress、resource/execution lease 和四种 lifecycle status。 | **K**：状态合法性矩阵必须集中在 State owner；拆开会产生状态真相分裂。 |

## 明确可化简项目的目标设计

下面展开 `A` 项的目标设计，并保留讨论后改判为 `K` 的决策记录；实施项必须可以据此写出明确设计和测试，不能停留在泛泛的“可以重构”。

### 1. Interrupt resume admission：一个 owner、一条现行语义（#11/#12/#38/#39 已完成）

目标调用链：

```text
canonical interrupt actions
  -> exact current InterruptedGraphNode + interrupt ID
  -> resume-input evidence
  -> exact reducer successor
  -> routing availability
  -> PlannedResume + admitted input + recovery fact
```

具体约束：

1. `plan_resumes` 只负责按 scope 编排；`prepare_resume` 只校验一个 scope 的 exact interrupt action、codec 和 input evidence；唯一 reducer 生成 exact successor。跨 scope admission 和 recovery proof 消费这同一个 successor，不模拟第二份 frontier。
2. canonical action、duplicate、current settlement、interrupt ID、codec、successor 和 availability 的检查顺序固定，不以“更早返回”换取更短代码。
3. `SnapshotMismatchError`（状态/坐标/证据）和 `GraphValueUnavailableError`（历史值不可得）继续各自归属原边界。
4. 删除旧的重复 normalization/lookup 路径后，才允许增加少量窄 record；不得把所有 action 或 frame 塞进一个宽 `InvocationContext`。
5. terminal FAILED 切片已经删除 failed resume、failed-node override、skip、output substitution 和 `SkippedGraphNode` 的全部生产路径；现行 resume 只可能用 override input 把 exact `InterruptedGraphNode` 改回 Pending，不保留兼容 alias 或隐藏 action。

最小行为测试集：多 scope canonical order、重复 action、未知 scope/node、非 interrupted settlement、interrupt ID 错误、codec 错误、不可用历史 input，以及每一种错误的优先级。

#38/#39 的实现只新增一个私有、不可变的 `_PlannedLineage` 索引记录；记录里仍是原有 `GraphRunState`，没有第二套运行时状态。它只保存 canonical bindings，fence/resume/continuation 校验共用同一份查找规则；interrupt resume action 先按 canonical 顺序逐组迭代 `groupby`，再交给现有 reducer。实现已验收，错误顺序、恢复语义和唯一 reducer 边界不变。
...
| invocation | fresh、state-only recovered、complete/recovered continuation、interrupt resume、fence、preflight 条件 |
| owner admission | fresh root/child、continued root/child、terminal child reconstruction、partial commit、handoff failure |
| scope state machine | completed、failed、aborted、settled routing、awaiting interrupt、active lease error、executable、children waiting |
...
| nested execution | missing/start、active/drive、completed/failed/aborted/awaiting interrupt、boundary installation、retirement、evidence handoff |
| recovery proof | live/quiescent、routing、resource selection、nested completed/failed/aborted/awaiting 组合、terminal/limit、worklist 去重/预算 |
| state | start、claim、fence、settle success/failure/interrupt（failure 可原子进入 terminal FAILED）、interrupt resume、advance、complete、abort、resource acquire/release/replay |
| exit | completed/failed/aborted/awaiting-interrupt result、continuation、node/commit/invocation cancellation、abort/release cleanup |
...
| 入口分支 | 当前行为 | 是否是偶然复杂度 |
|---|---|---|
| `values` 与 `state/continuation/interrupt resume` 同时出现 | 立即 `SnapshotMismatchError` | 否；这是 API mode 的互斥边界 |
...
| state-only（无 continuation） | 建立空 evidence，标记 `recovered=True`；terminal FAILED 直接按持久状态投影诊断，live state 在 claim 前做 recovery proof | 语义必要；恢复证据不完整，不能伪装成 complete continuation |
| continuation | 验证 family identity/state、读取 child/frame evidence，再区分 recovered/complete | 语义必要；可把验证阶段从入口移出 |
| interrupt resume 非空 | canonical order、fence、exact interrupt ID、candidate successor、override input、recovery proof | 规则必要；目前由多个窄 owner 分担，入口只应编排 |
...
### 2. 编译链：全部可达，`_compile_graph` 的可证明局部净化已完成
...
共同路径
  ...
  -> plan_resumes
     -> interrupt action canonical/duplicate/scope resolution
     -> exact current InterruptedGraphNode + interrupt ID
     -> resume input evidence + exact reducer successor
  -> admit_state_owned_overrides
  -> recovered 或有 interrupt resume 时 preflight_recovery
...
terminal FAILED state 也走同一个 state-led admission 入口，但 recovery proof 和 driver 都把它视为已终止事实：不生成 resume candidate，不 claim，不调用 callable。failed/skip/substitution 的旧动作、证据和 reducer 分支已经整体删除。
...
prepare_superstep
  ...
  -> state.status == FAILED          -> FailedGraph
  -> state.status == ABORTED         -> AbortedGraph
  -> frontier == SETTLED             -> resolve_routing -> ReadyToResolve
  -> frontier == FAILED              -> FailedGraph
  -> frontier == AWAITING_RESUME     -> AwaitingResume(interrupted IDs)
...
这棵树的每个叶子都是一个持久状态或一个明确的领域错误，顺序也体现了 terminal、routing、interrupt resume、lease 的优先级。`frontier_status` 在仍有 Pending 时先返回 EXECUTABLE，让同一 claim 已经启动的 completion 排空；Pending 全部结算后，Failed 优先于 Interrupted，最后一次 settlement 在同一 reducer commit 中把 run 原子转为 FAILED。FAILED 不进入 routing。它目前**足够简单，应保留**。重复的 snapshot guard/`plan_tasks` 检查是分层 fail-closed 边界：入口、executor 和 reducer 各自验证自己接收的证据，不能仅因文本相似就删除。
...
_GraphRun.drive_quantum
  while
    ...
    ├─ WaitingForChildren
    │  ├─ 已有 failed child   -> abort 仍在 awaiting 的 sibling child -> 父节点提交 TaskFailure -> 下一轮
    │  ├─ missing           -> 逐个 _start_child -> 下一轮
    │  ├─ active            -> 逐个 _drive_child -> 下一轮
    │  └─ 没有可推进 child   -> AwaitingResume(())
    └─ Completed/Failed/Aborted/AwaitingResume -> 返回 boundary
```

这是整个 execution engine 唯一的 live scope state machine。把每个叶子拆成一个 dispatcher、再造一个 recovery/runtime runner，都会让状态转移的 owner 分裂，反而不易维护。`AwaitingResume(())` 明确表示“当前 scope 没有本地 interrupt，但至少一个 descendant scope 正在等待 interrupt resume”；child 已先 handoff durable evidence，最终 public result 从完整 lineage 汇总真实 interrupt views。它不是另一种状态或隐藏 sentinel，无需再造 `ChildrenQuiescent` 分支。
...
最后一个 node settlement
  -> SettleGraphNode commit
     ├─ frontier 含 Failed
     │  -> 同一 reducer commit 原子写 GraphRunStatus.FAILED
     │  -> 清除 lease/resources，保留诊断 frontier；不进入 routing
     └─ frontier 全部 Succeeded -> frontier 变为 SETTLED
...
successful settlement 和 routing 分成两个 command/commit 是恢复屏障：前者记录事实，后者依据已提交事实作决定。failure termination 不解释 topology，所以最后一次 failure settlement 直接在 State owner 中关闭 run；这不是 routing 快捷路径，也没有第二个 `FailGraphRun` command。不能为了少一个函数或少一次循环，把 successful routing decision 塞进 `settle_graph_node`。#15/#16 已经在 routing owner 内按 accumulation、required-target 和 diagnostics 三个连续区块表达；继续拆 helper 不会净删规则，因此评审后保留。
...
| `SettleGraphNode` | success/failure/interrupt | settlement、resource release、lease 清除原子完成；最后一个 Pending 结算后，Failed 优先并在同一 commit 进入 terminal FAILED |
| `ResumeGraphNodes` | interrupt only | exact Interrupted settlement、interrupt ID、override input/codec 约束原子更新 |
...
dispatch 前统一验证旧 state 和 revision，叶子构造新 state，最终只在 reducer 中 `revision + 1`。`GraphRunStatus` 的四种状态是 RUNNING、COMPLETED、FAILED、ABORTED；FAILED 由 `SettleGraphNode` 叶子产生，不增加平行 public reducer。把分支拆成多个 public reducer 或让 execution 直接写 `GraphRunState` 都会破坏唯一真相；#43–#49 的多数 K 结论经完整链路复核后不变。
...
admit_continued_root.admit_children
  ...
  ├─ terminal child
  │  -> 从 boundary frame 重建 CompletedChild，或读取 FailedChild/AbortedChild
...
parent._drive_child
  -> opaque_handle.drive
     ...
     ├─ AwaitingResume（interrupt only）-> handoff evidence 一次 -> parent 标记等待
     └─ terminal
        -> terminal_projection
        -> terminal_boundary（completed 才允许 output；failed 投影为父节点 TaskFailure）
        -> handoff evidence 一次
...
recovery 只做无副作用 reachability proof，复用 `plan_tasks`、`resolve_routing_facts`、settlement projection 和 reducer；它没有另一套状态写入或 session。worklist、availability fixed point、递归 child outcome 和错误优先级是算法必要复杂度，当前**足够简单且应保留**。控制回环在 publication 可用性不再增长时折叠为不动点，不靠展开到 `max_supersteps` 才收敛，也没有抬高 proof budget。把 recovery 与 runtime 粗暴合并，会同时破坏“先证明、后提交”和唯一 execution engine 两条架构约束。

逐叶检查还包括：已有/新 child state、completed output history、failed/aborted child、awaiting interrupt、execution-limit、nested completed/failed/aborted variations、FAILED 优先于 awaiting sibling、callable success route 分叉、resource-selected live set、routing abort 中“真实业务不可达”与“历史 frame 缺失”的区别，以及 4,096 transfer-state budget。没有发现可以用一个线性流程等价替换这些状态空间分支的设计。

### 10. boundary、结果和 cleanup：出口与 admission cleanup 政策均已明确
...
drive boundary
  -> project_graph_result
     ...
     ├─ FailedGraph    -> 至少一个 failure view + 可选 interrupt diagnostics
     ├─ AbortedGraph  -> 要求 canonical abort -> abort view
     └─ AwaitingResume -> 只汇总 root/child interrupt views
  -> finish
...
| continuation/frame/interrupt-resume admission | 是 | 顺序保留；共享不可变 canonical lineage index，并按需形成 scope action groups，frame 先结构后语义；failed/skip/substitution 已删除 | #11/#12/#38/#39/#40 已完成；#41 K |
| `prepare_superstep` 状态分支 | 是 | 状态叶子和错误顺序清晰，不再泛化 dispatcher | #4、#8、#17、#25 等 K |
| `drive_quantum` 唯一 runner | 是 | 保留循环；`AwaitingResume(())` 是 descendant interrupt 的 scope-local 投影，真实 interrupt view 由 lineage 汇总 | #25 K |
...
`B` 项的共同风险是“看起来拆开了，实际增加了 owner/record/调用链”。本轮曾列入 B 的项目均已完成有净收益的局部整理，或在复审后改判为 K；当前没有为了指标而必须继续实施的 B 项：

| 组 | 项目 | 复审结论 |
|---|---|---|
| snapshot/definition/resource validation | #35、#46 | 改判 K：结构检查、compiled compatibility、durable replay 各有唯一 owner，异常边界不交叉；继续拆分没有净收益。 |
| compiler proof | #32 | 已完成局部净化：删除重复 canonicalization、临时边界对象和第二次节点遍历，未引入未经证明的 typed phase result。#30 改判 K：其交叉循环就是联合路径证明本身。 |
