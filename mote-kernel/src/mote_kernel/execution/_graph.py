...
class Graph(Generic[InputT, OutputT]):
    ...
    def add_resource(self, resource_id: str) -> Self:
        """Declare one exclusive resource in deterministic declaration order."""

        self._require_mutable()
        self._resources.append(ResourceDefinition(ResourceId(resource_id), len(self._resources)))
        return self
        ...
        *,
        ...
        self._nodes.append(
            NodeDefinition(
                ...
                tuple(ResourceId(resource_id) for resource_id in resources),
            )
        )
        return self
