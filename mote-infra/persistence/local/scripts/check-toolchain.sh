...
actual_rust="$(cd "$ROOT" && rustc --version | awk '{print $2}')"
actual_cargo="$(cd "$ROOT" && cargo --version | awk '{print $2}')"
...
command -v rustfmt >/dev/null || {
    printf 'toolchain: rustfmt is unavailable\n' >&2
    exit 1
}
command -v cargo-clippy >/dev/null || {
    printf 'toolchain: cargo-clippy is unavailable\n' >&2
    exit 1
}
