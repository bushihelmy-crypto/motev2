...
[[ -f "$ROOT/Cargo.lock" ]] || {
    printf 'cargo hygiene: Cargo.lock is required\n' >&2
    exit 1
}
[[ -f "$ROOT/.gitignore" ]] || {
    printf 'cargo hygiene: .gitignore is required\n' >&2
    exit 1
}
...
grep -q '"name":"mote-infra-persistence-local"' <<<"$metadata" || {
...
if find "$ROOT" \
    ...
    -type f \( -name '*.crate' -o -name '*.rlib' -o -name '*.rmeta' \) -print -quit | grep -q .; then
